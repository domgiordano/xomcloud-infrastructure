...
resource "aws_cloudwatch_log_group" "ecs_download_service" {
  name              = "/ecs/${var.app_name}-download-service"
  retention_in_days = 7

  tags = merge(local.standard_tags, tomap({"name" = "${var.app_name}-download-service-logs"}))
}
